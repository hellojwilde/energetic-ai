/**
 * @license
 * Copyright 2023 Jonathan Wilde. All Rights Reserved.
...
export class KNNClassifier {
  ...
  addExample(example: Tensor, label: number | string): void {
    ...
    tidy(() => {
      const normalizedExample = this.normalizeVectorToUnitLength(
        reshape(example, [example.size])
      );
      const exampleSize = normalizedExample.shape[0];

      if (this.classDatasetMatrices[label] == null) {
        this.classDatasetMatrices[label] = reshape(normalizedExample, [
          1,
          exampleSize,
        ]);
      } else {
        const newTrainLogitsMatrix = concat<Tensor2D>(
          [
            reshape(this.classDatasetMatrices[label], [
      ...
      keep(this.classDatasetMatrices[label]);
  ...
  private similarities(input: Tensor): Tensor1D {
    return tidy(() => {
      ...
      keep(this.trainDatasetMatrix);

      const numExamples = this.getNumExamples();
      return reshape(
        matMul(
          reshape(this.trainDatasetMatrix, [numExamples, exampleSize]),
          reshape(normalizedExample, [exampleSize, 1])
        ),
        [numExamples]
      );
    });
  ...
  async predictClass(
    ...
    const knn = tidy(() => cast(this.similarities(input), "float32"));
